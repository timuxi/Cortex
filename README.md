...
*   **极致轻量 MoE 架构**：面回归混合专家（MoE）设计，模型总参数量仅 0.1B，前向推理激活参数低至 ~67M，实现极低算力设备上的超高推理吞吐。。
...
*   **全链路训练代码 100% 开源**：开放全生命周期训练流，涵盖：Pretrain（预训练）、Midtrain（中继训练）、SFT（指令微调）、DPO（偏好优化） 以及 PPO（强化学习）。
*   **高度解耦**：
